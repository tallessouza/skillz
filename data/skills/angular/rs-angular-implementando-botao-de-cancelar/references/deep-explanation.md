...
No gerenciador de filmes da Skillz, a tela de criacao de filme tem um botao "Cancelar" que simplesmente volta para a tela de explorar (`/explorar`). O instrutor removeu o metodo `cancelar()` do componente e colocou `[routerLink]="['/explorar']"` diretamente no botao do template.
