...
O instrutor demonstrou um caso real: no site Skillz Help Center, textos como "e-explorer", "r-skillz", "d-discover" eram lidos caractere por caractere pelo leitor. Esses prefixos simbólicos são puramente visuais/decorativos. A solução é `aria-hidden="true"` nos prefixos OU `aria-label` no container.
...
O caso do Help Center da Skillz é emblemático: visualmente, "e-explorer" parece um prefixo estilizado. Ninguém perceberia o problema sem ouvir o leitor de tela soletrando "e-explorer" caractere por caractere. Essa é a razão principal para testar com leitores: problemas de acessibilidade frequentemente são invisíveis para quem enxerga.