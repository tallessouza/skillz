...
O instrutor (Diego, Skillz) explica que usar um estado React puro (`useState`) para controlar se a sidebar está aberta ou não "não é tão interessante" porque o estado não é **acessível**. O Radix UI Collapsible fornece:
