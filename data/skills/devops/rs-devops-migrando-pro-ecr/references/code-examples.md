...
- name: Build and Push
  ...
  with:
    push: true
    tags: danielrodrigues/skillz-ci:latest
...
- name: Build and Push to ECR
  run: |
    docker build -t 123456789.dkr.ecr.us-east-2.amazonaws.com/skillz-ci:$TAG .
    docker push 123456789.dkr.ecr.us-east-2.amazonaws.com/skillz-ci:$TAG
```
...
```
ECR > Repositories > skillz-ci > Image tags
...
- name: Build and Push to ECR
  env:
    ...
    TAG: ${{ steps.generate-tag.outputs.tag }}
  run: |
    docker build -t $REGISTRY/skillz-ci:$TAG -t $REGISTRY/skillz-ci:latest .
    docker push $REGISTRY/skillz-ci:$TAG
    docker push $REGISTRY/skillz-ci:latest
```
