...
- name: Tag image with registry prefix
  run: docker tag skillz-ci-api:${{ github.sha }} ${{ secrets.DOCKERHUB_USERNAME }}/skillz-ci-api:${{ github.sha }}
...
docker pull danielrodrigues/skillz-ci-api:abc123def

# Verificar tags disponiveis (via Docker Hub API)
curl -s "https://hub.docker.com/v2/repositories/danielrodrigues/skillz-ci-api/tags" | jq '.results[].name'
```
