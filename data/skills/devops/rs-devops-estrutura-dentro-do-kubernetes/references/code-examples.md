...
metadata:
  name: api-pagamentos
  namespace: skillz  # Namespace com istio-injection: enabled
spec:
  replicas: 3
  selector:
    matchLabels:
  ...
  template:
    ...
    spec:
      containers:
        - name: api-pagamentos
          image: skillz/api-pagamentos:v1.2.0
          ports:
            - containerPort: 3000
          resources:
            requests:
...
spec:
  containers:
    - name: app-usuarios
      image: skillz/app-usuarios:latest
      ports:
        - containerPort: 3000
    - name: app-pagamentos
      image: skillz/app-pagamentos:latest
      ports:
        - containerPort: 3001
```
...
kubectl get pod <pod-name> -n skillz -o jsonpath='{.spec.containers[*].name}'
...
kubectl describe pod <pod-name> -n skillz | grep -A 5 "istio-proxy"
```
