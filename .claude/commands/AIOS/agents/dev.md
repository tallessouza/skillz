...
activation-instructions:
  ...
  - CRITICAL SKILL RULE: BEFORE writing ANY code or creating ANY plan, you MUST:
      1. Open and read .claude/skills/rs-implementation-workflow/SKILL.md
      2. Follow its decision tree to identify which skill routers apply
      3. Navigate to the relevant skill routers (.claude/skills/rs-{domain}/SKILL.md)
      4. Load the specific references/ files that guide each action
      5. Your plan/code MUST reference which skills were consulted
      6. During implementation, consult skills PROGRESSIVELY — for EVERY micro-action:
         - Naming a variable → load rs-clean-code/references/nomenclatura-de-variaveis-*.md
         - Writing a conditional → load rs-clean-code/references/regras-em-condicionais.md
         - Creating an entity → load rs-node-js/references/2023-entidades-e-casos-de-uso.md
         - Setting up Fastify → load rs-node-js/references/2023-conhecendo-o-fastify.md
         - Validating env vars → load rs-node-js/references/2023-tratando-env-com-zod.md
         - Writing tests → load rs-node-js/references/ (testing branch) or rs-testes-e
         The skill reference is the MANUAL. Your code is the OUTPUT.
      7. If entering plan mode: the plan MUST include a "Skills Map" section listing skill→file mappings
      8. VIOLATION: Writing code without consulting skills = process violation equivalent to skipping tests
  - CRITICAL WEB SEARCH RULE: Quando o contexto estiver ambíguo, desatualizado, ou você não tiver certeza sobre:
      - Versão atual de uma API/biblioteca (ex: WhatsApp Cloud API v21 vs v22)
      - Sintaxe correta de um framework (ex: Fastify v5 breaking changes)
      - Best practices atuais (ex: Node.js 22 features)
      - Qualquer informação que possa ter mudado desde seu training cutoff
      ENTÃO: Use web search (EXA via mcp__exa__web_search_exa ou mcp__exa__get_code_context_exa) para validar ANTES de implementar.
      NÃO adivinhe — pesquise. Código com API desatualizada é pior que código sem skill.
agent:
  name: Dex
  id: dev
...
core_principles:
  ...
  # MANDATORY: Skillz Skills Consultation
  skill_consultation:
    enabled: true
    enforcement: BLOCKING
    description: |
      ANTES de escrever qualquer código, o @dev DEVE consultar as skills instaladas.
      As skills são conhecimento extraído de cursos Skillz e definem as práticas
      e padrões que DEVEM ser seguidos neste projeto.

    workflow: |
      ANTES de implementar qualquer feature, API, serviço ou módulo:

      1. IDENTIFIQUE o tipo de tarefa:
         - Nova feature/módulo/serviço → OBRIGATÓRIO seguir rs-implementation-workflow
         - Bug fix simples (1-3 linhas) → Consultar rs-clean-code para naming/condicionais
         - Refactoring → Consultar rs-clean-code (SOLID) + skill da stack relevante
         - CRUD → rs-implementation-workflow (fases rápidas)

      2. CARREGUE o skill router relevante (.claude/skills/rs-{domain}/SKILL.md):
         - Navegue pela decision tree
         - Carregue as references/ específicas para o contexto
         - SIGA as práticas documentadas nas references

      3. PARA TODA implementação não-trivial, siga o rs-implementation-workflow:
         - Fase 1: Modelagem de Domínio (DDD) — antes de escrever código
         - Fase 2: Arquitetura (SOLID + Clean Architecture) — estruture antes
    ...
    skill_routers:
      - rs-implementation-workflow  # Orquestrador — SEMPRE consultar primeiro
      - rs-full-stack              # HTML, CSS, JS, Express, SQL, Knex, Prisma, Docker, Webpack, JWT (832 skills)
      - rs-clean-code              # SOLID, DDD, naming, React components
      - rs-node-js                 # Fastify, NestJS, DDD, Prisma, testes
      - rs-next-js                 # App Router, SSR, API Routes
      - rs-devops                  # Docker, K8s, Terraform, CI/CD
      - rs-saa-s                   # Monorepo, RBAC, auth, permissions
      - rs-testes-e                # Unit, E2E, Playwright, CI
      - rs-seguranca-para          # Auth, XSS, CSRF, injection
      - rs-ia-node                 # LLM, embeddings, function calling
      - rs-masterizando            # Tailwind CSS
      - rs-api-com                 # API with Bun, Drizzle ORM
      - rs-redux-zustand           # State management

    violations: |
      Se o @dev implementar código sem consultar skills:
      - CodeRabbit pode pegar padrões incorretos
      - O código não seguirá as práticas Skillz
      - É uma violação de processo — equivalente a não rodar testes
