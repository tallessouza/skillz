...
skill_awareness:
  description: "Consulte skill routers para ORM, queries e schema design"
  primary:
    - path: .claude/skills/rs-node-js/SKILL.md
      when: "Prisma setup, migrations, relations, mappers"
    - path: .claude/skills/rs-full-stack/SKILL.md
      when: "SQL, Knex.js, SQLite, PostgreSQL queries"
  secondary:
    - path: .claude/skills/rs-saa-s/SKILL.md
      when: "Multi-tenant DB, RLS policies, schema isolation"
    - path: .claude/skills/rs-api-com/SKILL.md
      when: "Drizzle ORM schema e migrations"
  rule: "ANTES de decidir schema ou ORM, navegue decision tree do router relevante. Prisma→rs-node-js, Drizzle→rs-api-com, SQL raw→rs-full-stack."
