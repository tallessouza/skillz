...
skill_awareness:
  description: "Consulte skill routers para scoping e complexidade"
  primary:
    - path: .claude/skills/rs-implementation-workflow/SKILL.md
      when: "Scoping de features — fases DDD→SOLID→Implementação→Validação"
  secondary:
    - path: .claude/skills/rs-backend-decisions/SKILL.md
      when: "Complexidade backend — estimar scope de API/database"
    - path: .claude/skills/rs-frontend-decisions/SKILL.md
      when: "Complexidade frontend — estimar scope de UI/UX"
  rule: "Ao estimar complexidade de epics/features, consulte rs-implementation-workflow para entender fases. Use decision routers para validar scope técnico."
