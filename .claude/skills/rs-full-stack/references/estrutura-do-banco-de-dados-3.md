...
metadata:
  ...
  tags: [sql, database, schema-design, relationships, uuid]
---
...
|----------|---------------|---------|
| Foreign key constraint falha ao inserir | Registro pai nao existe na tabela referenciada | Inserir o registro pai primeiro (ex: user antes de delivery) |
| UUID duplicado ao inserir | Geracao de UUID nao e unica | Usar funcao do banco (`gen_random_uuid()` no Postgres) ou biblioteca confiavel |
| Status sobrescrito sem historico | Campo status atualizado diretamente | Criar tabela de logs separada para registrar cada mudanca |
| Erro ao criar tabela com REFERENCES | Tabela referenciada ainda nao existe | Criar tabelas na ordem correta: pais antes de filhas |
