...
metadata:
  ...
  course: full-stack
  module: fundamentos
  tags: [git, github, repository, push, remote, version-control]
---
...
|----------|---------------|---------|
| `fatal: remote origin already exists` | Remote já configurado de tentativa anterior | `git remote set-url origin <url>` para atualizar |
| `error: src refspec main does not match any` | Branch local é `master`, não `main` | `git branch -M main` antes do push |
| Push rejeitado com `non-fast-forward` | Repo criado com README/license no GitHub | Crie repo vazio ou `git pull --rebase origin main` |
| `Permission denied (publickey)` | SSH key não configurada | Configure SSH key ou use HTTPS com token |
| Arquivos sensíveis aparecendo no GitHub | `.gitignore` não configurado antes do commit | `git rm --cached <arquivo>`, adicione ao `.gitignore`, faça novo commit |
