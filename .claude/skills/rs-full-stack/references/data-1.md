...
metadata:
  ...
  course: full-stack
  module: react-forms
  tags: [react, react-hook-form, Controller, date-input, forms, TypeScript]
---
...
|----------|---------------|---------|
| Warning "changing uncontrolled to controlled" | `defaultValues` nao definido ou `date: undefined` | Use `date: ""` como valor padrao |
| Input date nao atualiza | Controller nao configurado corretamente | Verifique se `{...field}` esta no spread do input |
| Tipo `Date` causa erro no submit | Input HTML retorna string, nao Date | Tipe o campo como `string` no FormData |
| Valor nao aparece no input | `field.value` nao conectado | Use `{...field}` para repassar todas as props |
