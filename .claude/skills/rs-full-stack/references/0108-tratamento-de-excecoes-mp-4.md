...
metadata:
  ...
  course: full-stack
  module: fundamentos
  tags: [express, error-handling, AppError, zod, middleware, validation]
---
...
### Problem: Async route errors crash the application instead of returning error response
- **Cause**: Express does not natively catch rejected promises in async route handlers
- **Fix**: Install and import `express-async-errors` at the top of `app.ts` before any route definitions — it patches Express to forward async errors to the error middleware
