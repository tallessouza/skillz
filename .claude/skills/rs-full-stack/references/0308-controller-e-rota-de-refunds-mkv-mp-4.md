...
metadata:
  ...
  course: full-stack
  module: fundamentos
  tags: [express, controller, router, crud, rest-api, scaffolding]
---
...
|----------|---------------|---------|
| `Cannot GET /refunds` (404) | Rota nao registrada no index | Adicionar `routes.use("/refunds", refundsRoutes)` no index |
| `Router is not a function` | Import incorreto do Express | Usar `import { Router } from "express"` |
| Controller retorna undefined | Metodo nao retorna response | Adicionar `return response.json(...)` no metodo |
| Rota privada acessivel sem token | Registrada antes do middleware de auth | Mover para depois de `routes.use(ensureAuthenticated)` |
| Insomnia retorna 404 | URL ou variavel de environment incorreta | Verificar `{{ base_url }}/{{ resource }}` e o JSON do environment |
