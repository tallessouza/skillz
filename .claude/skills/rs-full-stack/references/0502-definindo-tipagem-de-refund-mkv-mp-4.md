...
metadata:
  ...
  course: full-stack
  module: fundamentos
  tags: [typescript, dto, enum, generics, api-typing]
---
...
|----------|---------------|---------|
| Sem autocomplete apos tipar a requisicao | Generic nao passado no `api.get` | Use `api.get<RefoundsPaginationAPIResponse>(...)` |
| Enum nao reconhecido no arquivo `.d.ts` | Enums precisam de arquivo `.ts`, nao `.d.ts` | Mova enums para arquivos `.ts` ou use `const enum` em `.d.ts` |
| Tipo do campo nao corresponde ao retorno da API | Shape do DTO diverge do backend | Compare o DTO com a resposta real da API usando `console.log` |
| Tipos nao reutilizaveis entre componentes | Tipagem inline no componente | Mova para arquivo separado em `DTOs/` com sufixo `APIResponse` |