...
metadata:
  ...
  course: full-stack
  module: fundamentos
  tags: [react, context, authentication, state-management, routing, typescript]
---
...
|----------|---------------|---------|
| Rotas não mudam após login | `save` não está chamando `setSession` | Verificar que `save(data)` chama `setSession(data)` internamente |
| `save is not a function` no componente | `save` não está no value do Provider | Adicionar `save` ao objeto `value={{ session, save }}` |
| Tela fica em branco após login | Switch/case não cobre o role retornado | Verificar se o role da API bate com os cases do switch |
| Constante de teste ainda controla rotas | Estado do contexto não substituiu a constante | Remover constante e usar `useAuth()` para obter session |
| TypeScript reclama de `save` no contexto | Tipo do contexto não inclui `save` | Adicionar `save: (data: UserApiResponse) => void` na interface |
