...
metadata:
  ...
  version: 1.0.0
  course: full-stack
  module: node-api-tools
  tags: [insomnia, api-testing, http-client, environment-variables]
---
...
|----------|---------------|---------|
| Variavel `{{ base_url }}` nao resolve | Ambiente nao selecionado como ativo | Selecione o environment `Dev` no dropdown do Insomnia |
| Request retorna erro de conexao | API nao esta rodando | Inicie o servidor com `npm run dev` antes de testar |
| URL resolve para valor errado | Variavel `resource` no folder environment incorreta | Verifique o JSON do folder environment da pasta |
| Resposta 404 mesmo com API rodando | Rota ou metodo HTTP incorreto | Confira se o metodo (GET/POST) e o path correspondem a API |
