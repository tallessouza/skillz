...
metadata:
  ...
  tags: [express, error-handling, middleware, AppError, async-errors]
---
...
|----------|-------|---------|
| Erros async nao sao capturados | Falta `express-async-errors` | Instale e importe ANTES de qualquer rota |
| Middleware de erro nao executa | Registrado ANTES das rotas | Mova `app.use(errorHandling)` para DEPOIS de todas as rotas |
| `instanceof AppError` sempre retorna false | AppError importado de caminho diferente | Verifique se ha apenas UMA classe AppError e todos importam dela |
| Resposta retorna HTML em vez de JSON | Express error handler padrao esta respondendo | Garanta que o middleware customizado tem 4 parametros: `(error, req, res, next)` |
