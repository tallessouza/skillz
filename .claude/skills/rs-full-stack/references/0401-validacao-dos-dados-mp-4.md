...
metadata:
  ...
  course: full-stack
  module: fundamentos
  tags: [zod, react, form-validation, error-handling, loading-state, client-side]
---
...
|----------|---------------|---------|
| Valor monetário com vírgula falha na validação | Vírgula brasileira não convertida | Aplicar `amount.replace(",", ".")` antes do parse |
| Botão de submit fica travado em loading | `setIsLoading(false)` não está no `finally` | Mover reset de loading para bloco `finally` |
| Todos os erros mostram mesma mensagem genérica | Faltou check de `instanceof ZodError` | Verificar `error instanceof z.ZodError` antes do catch genérico |
| Primeiro campo inválido não é destacado | Usando `error.message` em vez de `error.issues` | Usar `error.issues[0].message` para o primeiro erro de validação |
| Schema recriado a cada render | Schema definido dentro do componente | Mover schema para fora do componente, como constante do módulo |
