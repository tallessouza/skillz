...
metadata:
  ...
  course: full-stack
  module: fundamentos
  tags: [multer, upload, config, crypto, mime-type, express]
---
...
|----------|---------------|---------|
| `ENOENT` ao fazer upload | Pasta `tmp/` não existe | Crie a pasta manualmente ou use `fs.mkdirSync(TMP_FOLDER, { recursive: true })` |
| Arquivo sobrescrito por outro upload | Nome original sem hash único | Use `crypto.randomBytes(10).toString("hex")` como prefixo |
| Upload rejeitado sem mensagem clara | Arquivo excede `FILE_SIZE` | Verifique o limite configurado e trate o erro do Multer |
| `__dirname is not defined` | Projeto usando ESM sem CommonJS | Use `import.meta.url` com `fileURLToPath` ou `path.dirname` |
