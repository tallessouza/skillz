...
metadata:
  ...
  course: full-stack
  module: fundamentos
  tags: [express, authorization, listing, middleware, rbac, rest-api]
---
...
|----------|---------------|---------|
| Employee consegue listar todos os recursos | Middleware de autorização não aplicado | Adicionar `verifyUserAuthorization("manager")` na rota GET |
| `unauthorized` para Manager válido | Token desatualizado com role antigo | Fazer novo login para gerar token com role atualizado |
| Middleware aplicado em rotas que não deveria | `router.use(middleware)` aplicado no grupo inteiro | Usar middleware como argumento na rota específica em vez de `router.use` |
| Método `index` retorna 404 | Rota GET não registrada no router | Verificar `refundRoutes.get("/", ...)` no arquivo de rotas |
| Teste de autorização sempre passa | Testando apenas com token de Manager | Testar com tokens de ambos os perfis (Employee e Manager) |
