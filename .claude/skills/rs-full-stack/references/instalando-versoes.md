...
metadata:
  ...
  course: full-stack
  module: npm-versions
  tags: [npm, versions, dependencies, package-lock, installation]
---
...
|----------|-------|---------|
| `npm i` instala versao inesperada | Tag `latest` nao e a mais recente publicada | Verificar tags com `npm view pacote dist-tags` |
| package-lock.json desincronizado | Edicao manual do package.json sem rodar npm i | Executar `npm i` sem argumentos para sincronizar |
| node_modules corrompida | Instalacao parcial ou conflito | Deletar node_modules e package-lock.json, depois `npm i` |
| Versao beta instalada acidentalmente | Versao com tag `next` instalada | Reinstalar com versao estavel: `npm i pacote@latest` |
