...
metadata:
  ...
  course: full-stack
  module: fundamentos
  tags: [npm, semver, versioning, dependencies, package-json]
---
...
|----------|---------------|---------|
| Pacote atualizado quebrou o projeto | Major version com breaking changes | Consulte changelog, reverta com `git checkout package.json && npm install` |
| `npm install` instala versao diferente da esperada | Range no package.json permite variacao | Use versao exata (`"express": "4.19.2"`) ou verifique o range |
| semver.npmjs.com nao lista o pacote | Nome do pacote digitado incorretamente | Verifique o nome exato no npmjs.com |
| Conflito de versoes entre dependencias | Duas deps exigem versoes incompativeis da mesma lib | Use `npm ls <pacote>` para ver a arvore e resolva com `overrides` |
