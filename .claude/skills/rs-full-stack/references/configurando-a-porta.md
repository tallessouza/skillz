...
metadata:
  ...
  course: full-stack
  module: fundamentos
  tags: [environment-variables, zod, dotenv, node, server]
---
...
|----------|---------------|---------|
| Erro do Zod ao iniciar servidor | Variavel de ambiente obrigatoria ausente | Crie arquivo .env com as variaveis necessarias ou adicione .default() no schema |
| Porta sempre usa valor padrao | Variavel PORT vazia no .env em vez de removida | Remova a linha inteira do .env para usar o default do Zod |
| Tipo da porta incorreto (string) | Sem z.coerce.number() no schema | Use `z.coerce.number()` para converter string do env para number |
| .env commitado no repositorio | .env nao esta no .gitignore | Adicione `.env` ao .gitignore imediatamente e remova do tracking |
