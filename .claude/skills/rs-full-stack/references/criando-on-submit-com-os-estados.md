...
metadata:
  ...
  course: full-stack
  module: react-forms
  tags: [react, form, useState, onSubmit, controlled-inputs]
---
...
|----------|---------------|---------|
| Pagina recarrega ao submeter formulario | Faltou `e.preventDefault()` no handler | Adicione `e.preventDefault()` como primeira linha do handler |
| Input nao atualiza o estado | Faltou `onChange` no input | Adicione `onChange={(e) => setState(e.target.value)}` |
| Enter nao submete o formulario | Handler no `onClick` do botao em vez de `onSubmit` do form | Use `<form onSubmit={handleSubmit}>` e `<button type="submit">` |
| Botao submete multiplas vezes | Faltou estado `isLoading` com `disabled` | Adicione `const [isLoading, setIsLoading] = useState(false)` e `disabled={isLoading}` |
| Tipo do evento nao reconhecido | Tipo incorreto no TypeScript | Use `e: React.FormEvent<HTMLFormElement>` |