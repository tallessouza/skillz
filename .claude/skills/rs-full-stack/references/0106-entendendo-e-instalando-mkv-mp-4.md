...
metadata:
  ...
  course: full-stack
  module: fundamentos
  tags: [cors, cross-origin, express, security, middleware, frontend-backend]
---
...
### Problem: "Access to fetch blocked by CORS policy" error in browser console
- **Cause**: The API server does not have CORS middleware enabled, so the browser blocks cross-origin requests
- **Fix**: Install the `cors` package (`npm install cors`) and add `app.use(cors())` before your route definitions
