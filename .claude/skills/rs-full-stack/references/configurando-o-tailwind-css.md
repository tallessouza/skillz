...
metadata:
  ...
  course: full-stack
  module: css-styling
  tags: [tailwind-css, theme-customization, css-variables, design-tokens, fonts, colors]
---
...
|----------|---------------|---------|
| Cor customizada nao aparece | Nome da variavel nao segue o padrao `--color-{nome}-{peso}` | Corrigir formato da variavel no bloco `@theme` |
| Fonte nao muda na aplicacao | Import do Google Fonts ausente no HTML | Adicionar `<link>` da fonte no `<head>` do HTML |
| Tamanho customizado (`text-xxs`) nao funciona | Valor em `px` em vez de `rem` | Converter para rem: `valor / 16` |
| Bloco `@theme` ignorado | Posicionado antes do `@import "tailwindcss"` | Mover `@theme` para depois do `@import` |
