...
metadata:
  ...
  course: full-stack
  module: fundamentos
  tags: [zod, validation, forms, typescript, schema, refine]
---
...
### Problem: String with only spaces passes `z.string().min(1)` validation
- **Cause**: Missing `.trim()` before `.min()` — whitespace-only strings have length > 0
- **Fix**: Chain `.trim()` before `.min()`: `z.string().trim().min(1, { message: "Required" })`
