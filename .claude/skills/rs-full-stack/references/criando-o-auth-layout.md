...
metadata:
  ...
  course: full-stack
  module: react-layout
  tags: [react, react-router, outlet, tailwind, layout, auth]
---
...
|----------|---------------|---------|
| Outlet nao renderiza o conteudo da rota filha | Rotas filhas nao estao aninhadas dentro do Route do layout | Aninhe as rotas como filhas: `<Route element={<AuthLayout />}><Route path="sign-in" ... /></Route>` |
| Card nao centraliza na tela | Faltou `w-screen h-screen` ou `flex justify-center items-center` | Adicione as classes de centralizacao no container pai |
| Breakpoint `md:` nao funciona | Tailwind nao esta configurado no projeto | Verifique se Tailwind esta instalado e configurado corretamente |
| Layout aparece em todas as rotas | Layout aplicado no nivel raiz sem rotas especificas | Agrupe apenas as rotas de auth dentro do layout |
| Imagem do logo nao carrega | Caminho do import incorreto | Verifique o caminho relativo do asset SVG/PNG |