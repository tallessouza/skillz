...
```text
# Pesquisa rapida no Google com prefixo MDN
MDN background CSS
MDN addEventListener JavaScript
MDN flexbox

# DevDocs — busca unificada
devdocs.io → digitar "background" → observar badge da tecnologia
```
...
|----------|---------------|---------|
| MDN nao tem traducao para o termo buscado | Nem todas as paginas tem versao em portugues | Use a versao em ingles — desenvolva ingles instrumental gradualmente |
| DevDocs mostra resultados de tecnologia errada | Multiplas tecnologias usam termos iguais | Observe o badge/selinho de tecnologia ao lado do resultado |
| DevDocs nao funciona offline | Modo offline nao foi ativado | Acesse DevDocs > Settings > habilite offline mode e selecione tecnologias |
| Resultado do Google leva a sites desatualizados | Fontes nao-oficiais aparecem antes do MDN | Prefixe a busca com `MDN` (ex: `MDN background CSS`) |
