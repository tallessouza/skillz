...
metadata:
  ...
  course: full-stack
  module: fundamentos
  tags: [prisma, schema, models, enums, relationships, migrations]
---
...
### Problem: `prisma migrate dev` fails with "migration is not in a clean state"
- **Cause**: Conflict between existing migrations and schema changes, often from manual database edits
- **Fix**: In development, delete the `prisma/migrations/` folder and the `dev.db` file, then re-run `npx prisma migrate dev --name create-tables`
