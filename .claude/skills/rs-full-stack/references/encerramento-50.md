...
metadata:
  ...
  version: 1.0.0
  course: full-stack
  module: testes-controllers
  tags: [testes, controllers, api, jest, supertest, padrao]
---
...
|----------|---------------|---------|
| Setup repetitivo em cada teste de controller | Factories e helpers nao extraidos | Crie funcoes createUser() e authenticateUser() reutilizaveis |
| Teste falha com 401 Unauthorized | Token nao incluido no header da requisicao | Adicione .set("Authorization", "Bearer token") ao request |
| Teste de delete retorna 200 mas dado ainda existe | Soft delete em vez de hard delete | Verifique se a query busca registros com deletedAt null |
| Cobertura de testes baixa apesar de muitos testes | Testando apenas happy path | Adicione testes para erros de validacao, 404 e 403 |
