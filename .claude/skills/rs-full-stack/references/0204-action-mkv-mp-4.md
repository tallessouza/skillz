...
metadata:
  ...
  course: full-stack
  module: fundamentos
  tags: [react-19, form-action, formdata, forms, useState-elimination]
---
...
### Problem: formData.get() returns null for a field
- **Cause**: The input element is missing the `name` attribute
- **Fix**: Add `name="fieldName"` to every `<input>` that needs to be read via `formData.get("fieldName")`
