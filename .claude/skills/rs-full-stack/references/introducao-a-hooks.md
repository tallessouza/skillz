...
|----------|---------------|---------|
| Estado não atualiza na tela após `setState` | Usando mutação direta no estado em vez de criar novo valor | Sempre crie um novo objeto/array: `setState([...arr, newItem])` |
| `useEffect` roda em loop infinito | Dependência do array muda a cada render (objeto/array novo) | Memoize a dependência com `useMemo` ou extraia para fora do componente |
| Hook chamado condicionalmente causa erro | Hooks devem estar no top-level, nunca dentro de `if` ou `for` | Mova o hook para fora da condicional e controle o comportamento internamente |
