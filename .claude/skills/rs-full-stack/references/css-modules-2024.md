...
metadata:
  ...
  version: 1.0.0
  course: full-stack
  module: react-estilizacao
  tags: [react, css-modules, scoped-css, component-structure, styling]
---
...
|----------|-------|---------|
| Estilos nao aplicados no componente | Usando string literal em className | Use `className={styles.container}` ao inves de `className="container"` |
| `Cannot find module './styles.module.css'` | Arquivo nao tem extensao `.module.css` | Renomeie para `styles.module.css` (o `.module` e obrigatorio) |
| Estilos vazam para outros componentes | Importando CSS sem `.module` | Use `import styles from './styles.module.css'` com import nomeado |
| Classe com hifen nao funciona (`styles.my-class`) | Hifens nao sao validos como propriedade JS | Use `styles["my-class"]` com bracket notation ou renomeie para camelCase |
