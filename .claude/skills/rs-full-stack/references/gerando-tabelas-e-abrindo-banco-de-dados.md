...
metadata:
  ...
  course: full-stack
  module: database
  tags: [prisma, migration, database, prisma-studio, beekeeper, postgresql]
---
...
|----------|---------------|---------|
| Migration falha por conflito | Banco com dados ou migrations anteriores inconsistentes | Use `npx prisma migrate reset` (apaga dados — apenas em dev) |
| Prisma Studio nao abre | Porta 5555 ja em uso por outro processo | Encerre o processo na porta ou use `--port` alternativa |
| Tabelas nao aparecem no Beekeeper | Conectado ao banco ou schema errado | Verifique conexao e confirme schema `public` |
| `DATABASE_URL` nao encontrada | Arquivo `.env` ausente ou variavel mal formatada | Crie `.env` na raiz com `DATABASE_URL=postgresql://user:pass@host:port/db` |
