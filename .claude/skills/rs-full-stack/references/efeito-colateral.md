...
metadata:
  ...
  course: full-stack
  module: react
  tags: [react, useEffect, side-effects, hooks, state, dependencies]
---
...
|----------|---------------|---------|
| useEffect executa infinitamente | Dependencia muda a cada render (objeto/array novo) | Memoize com `useMemo` ou mova a criacao para fora do render |
| useEffect nao reage a mudanca de estado | Estado nao esta no array de dependencias | Adicione o estado ao array: `[count]` |
| Warning "missing dependency" no ESLint | Variavel usada dentro do useEffect nao declarada como dependencia | Adicione a variavel ao array ou use `useCallback` para funcoes |
| Contador fica negativo | Validacao de limite esta no useEffect em vez do handler | Mova o `if (count > 0)` para dentro do `handleRemove` |
