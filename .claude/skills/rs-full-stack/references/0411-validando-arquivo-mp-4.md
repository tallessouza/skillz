...
metadata:
  ...
  course: full-stack
  module: fundamentos
  tags: [zod, upload, validation, mime-type, file-size, refine]
---
...
|----------|---------------|---------|
| Zod rejeita arquivo válido | Schema não tem `.passthrough()` e Zod descarta props extras | Adicione `.passthrough()` ao final do `z.object()` |
| Mensagem de erro mostra valores antigos | Mensagem hardcoded, não interpolada da config | Use template literal com valores de `uploadConfig` |
| `request.file` é undefined | Multer middleware não executou antes do controller | Verifique a ordem dos middlewares na rota |
| Arquivo inválido permanece na pasta tmp | Catch não deleta o arquivo temporário | Adicione `diskStorage.deleteFile(filename, "tmp")` no catch |
