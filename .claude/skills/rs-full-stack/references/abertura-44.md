...
|----------|---------------|---------|
| Input nao atualiza ao digitar | Input controlado sem `onChange` handler | Adicione `onChange={e => setState(e.target.value)}` |
| Warning de controlled/uncontrolled | Mistura de estrategias no mesmo input | Escolha uma estrategia e mantenha — nao alterne entre controlado e nao controlado |
| FormData retorna `null` | Input sem atributo `name` | Adicione `name="campo"` em cada input do formulario |
| Muitos re-renders com formulario grande | Um `useState` para cada campo | Considere usar `FormData` no submit para evitar estado por campo |