...
metadata:
  ...
  course: full-stack
  module: fundamentos
  tags: [authorization, rbac, middleware, express, roles, access-control]
---
...
|----------|---------------|---------|
| `unauthorized` mesmo com perfil correto | Role no token não bate com o array | Verificar o role no JWT payload e o array passado ao middleware |
| `verifyUserAuthorization is not a function` | Import incorreto ou sem instanciação | Verificar que o módulo exporta a função e que está sendo chamada com `()` |
| Middleware não bloqueia sem autenticação | `request.user` não é verificado | Adicionar `!request.user` na condição antes de `!role.includes(...)` |
| Novo perfil adicionado mas sem acesso | Array da rota não inclui o novo perfil | Adicionar o novo perfil ao array: `["manager", "employee", "novo"]` |
| Role passado como string em vez de array | Parâmetro é string, não array | Sempre passar array: `verifyUserAuthorization(["manager"])`, nunca string |
