...
metadata:
  ...
  course: full-stack
  module: fundamentos
  tags: [express, controller, authentication, sessions, router, rest-api]
---
...
|----------|---------------|---------|
| `Cannot POST /sessions` (404) | Rota não registrada no index | Adicionar `routes.use("/sessions", sessionsRoutes)` no index |
| `TypeError: sessionsController.create is not a function` | Controller não foi instanciado | Usar `new SessionsController()` antes de referenciar métodos |
| `this` undefined dentro do método create | Método passado como callback sem bind | Usar arrow function ou `.bind(this)` ao registrar a rota |
| Rota de session exige autenticação | Registrada após middleware de auth | Mover para antes do `routes.use(ensureAuthenticated)` |
| Request/Response sem autocomplete | Tipos do Express não importados | Adicionar `import { Request, Response } from "express"` |
