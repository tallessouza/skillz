...
metadata:
  ...
  course: full-stack
  module: react-forms
  tags: [React, react-hook-form, useForm, defaultValues, controlled-input, forms]
---
...
|----------|---------------|---------|
| Warning "changing uncontrolled to controlled" | `useForm()` sem `defaultValues` | Adicione `defaultValues: { campo: "" }` |
| Campo mostra `undefined` no console | Valor padrao e `undefined` ou `null` | Use string vazia `""` como default |
| Submit via Enter nao funciona | Form sem `handleSubmit` ou sem botao submit | Configure `onSubmit={handleSubmit(handler)}` e adicione `<button type="submit">` |
| Dados do form retornam vazios | Campo nao registrado com `register` | Adicione `{...register("campo")}` no input |
