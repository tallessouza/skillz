...
metadata:
  ...
  version: 1.0.0
  course: full-stack
  module: react-css-animations
  tags: [react, css, animation, keyframes, git-branch]
---
...
|----------|-------|---------|
| Animacao nao executa | Classe CSS nao esta sendo aplicada condicionalmente | Verifique se o estado booleano esta mudando e a classe condicional esta correta |
| Animacao executa apenas uma vez e para | Estado nao e resetado | Adicione `setTimeout(() => setState(false), duration)` para resetar |
| Animacao treme continuamente | Estado nunca volta para `false` | Garanta que o setTimeout tem a mesma duracao da animacao CSS |
| Branch nao aparece no remote | Branch criada apenas localmente | Execute `git push -u origin feature/nome` para publicar |
