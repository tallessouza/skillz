...
metadata:
  ...
  course: full-stack
  module: fundamentos
  tags: [prisma, bcrypt, user-registration, password-hashing, express, api]
---
...
| Hash retorna objeto Promise em vez de string | Faltou `await` no `hash()` | Usar `await hash(password, 8)` |
| Erro de unique constraint no email | `findFirst` não foi feito antes do `create` | Adicionar verificação de duplicidade antes de inserir |
| Senha salva em texto plano no banco | `hash()` não foi chamado antes do `create` | Sempre criptografar com bcrypt antes de persistir |
| Resposta retorna o hash da senha | Objeto inteiro do user foi retornado | Usar `res.status(201).end()` ou excluir password do retorno |
