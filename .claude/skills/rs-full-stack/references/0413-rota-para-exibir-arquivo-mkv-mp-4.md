...
metadata:
  ...
  course: full-stack
  module: fundamentos
  tags: [express, static, upload, serve-files, middleware]
---
...
|----------|---------------|---------|
| Arquivo retorna 404 | Rota estática registrada depois das rotas dinâmicas | Mova `app.use("/upload", ...)` para antes de `app.use(routes)` |
| Imagem não carrega no browser | Extensão do arquivo não incluída na URL | Acesse com nome completo: `{BASE_URL}/upload/arquivo.jpg` |
| Pasta de uploads hardcoded | Path não vem de `uploadConfig` | Use `express.static(uploadConfig.directory)` centralizado |
| Arquivos de outros tipos não são servidos | `express.static` serve qualquer tipo automaticamente | Verifique se o arquivo existe na pasta configurada |
