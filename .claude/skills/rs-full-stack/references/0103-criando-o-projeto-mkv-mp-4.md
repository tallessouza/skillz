...
metadata:
  ...
  course: full-stack
  module: fundamentos
  tags: [express, typescript, project-setup, npm, dependencies]
---
...
### Problem: TypeScript type definitions installed as production dependencies
- **Cause**: Running `npm i @types/express` without the `-D` flag
- **Fix**: Reinstall with `npm i @types/express -D` — npm will automatically move it to `devDependencies`
