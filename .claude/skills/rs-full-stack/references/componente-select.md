...
metadata:
  ...
  course: full-stack
  module: fundamentos
  tags: [react, select, forms, tailwind, css, components]
---
...
|----------|---------------|---------|
| Seta nativa do browser ainda aparece | `appearance: none` nao aplicado ao select | Adicione `appearance: none` no CSS do select |
| Placeholder "Selecione" nao aparece | Option padrao sem `disabled hidden` | Adicione `<option value="" disabled hidden>Selecione</option>` |
| Validacao `required` nao funciona | Option padrao com value nao vazio | Use `value=""` na option padrao para que required funcione |
| Props do Input sobrando no Select | Manteve `type` ao converter de Input | Remova props irrelevantes como `type` ao adaptar componente |
