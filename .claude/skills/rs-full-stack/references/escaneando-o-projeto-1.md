...
metadata:
  ...
  tags: [npm, dependencies, outdated, semver, update, audit]
---
...
|----------|---------------|---------|
| `npm outdated` retorna vazio | Todas as dependencias estao atualizadas dentro do range | Nenhuma acao necessaria — projeto esta em dia |
| `npm outdated` falha com ERESOLVE | Conflitos de peer dependencies | Resolver conflitos de peers antes de atualizar |
| Aplicacao quebra apos `npm update` | Atualizacao introduziu breaking change nao prevista | Verificar changelog, reverter com `npm install package@version-anterior` |
| Latest mostra major diferente mas Wanted e igual ao Current | `package.json` trava a major version | Atualizar manualmente o range no `package.json` se quiser a nova major |
