...
metadata:
  ...
  tags: [prisma, schema, model, enum, relations, database]
---
...
|----------|---------------|---------|
| Erro "Both sides of relation must be defined" | Relacionamento definido em apenas um modelo | Adicione o campo inverso (ex: `deliveries Delivery[]` no User) |
| Coluna no banco com nome camelCase | Faltou `@map("snake_case")` no campo | Adicione `@map("nome_snake_case")` |
| Enum nao aceita novo valor | Valor nao adicionado ao enum no schema | Adicione o valor ao `enum` e rode `prisma migrate dev` |
| `updatedAt` nao atualiza automaticamente | Campo sem `@updatedAt` | Adicione `@updatedAt` ao campo e marque como optional com `?` |
| Tabela criada com nome singular | Faltou `@@map("nome_plural")` no model | Adicione `@@map("tabela_plural")` no final do model |