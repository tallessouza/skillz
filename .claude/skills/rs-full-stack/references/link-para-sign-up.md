...
metadata:
  ...
  course: full-stack
  module: tailwind-css
  tags: [tailwind, css, link, anchor, hover, navigation]
---
...
|----------|-------|---------|
| Hover nao muda a cor do link | Falta `transition` na classe | Adicione `transition ease-linear` junto com `hover:text-*` |
| Link nao centraliza | Elemento inline por padrao | Adicione `text-center` e garanta que o container permite |
| Cor do link nao aparece em fundo escuro | Contraste insuficiente | Use `text-gray-100` ou `text-gray-200` para fundos escuros |
| Margin-top nao funciona no link | Elemento inline nao aceita margin vertical | Adicione `block` ou `inline-block` ao link |
