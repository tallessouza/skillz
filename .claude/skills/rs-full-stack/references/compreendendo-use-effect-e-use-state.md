...
metadata:
  ...
  course: full-stack
  module: fundamentos
  tags: [react, hooks, useEffect, useState, lifecycle]
---
...
|----------|---------------|---------|
| useEffect causa loop infinito | Array de dependencias ausente ou incorreto | Adicione `[]` para rodar uma vez ou `[dep]` para dependencia especifica |
| Erro 'Hooks can only be called inside a component' | useEffect fora do corpo do componente | Mova o useEffect para dentro da funcao do componente |
| Estado nao atualiza na tela | Mutacao direta do estado em vez de usar setState | Use `setState(novoValor)` — nunca altere o estado diretamente |
| Erro com async no useEffect | Arrow function do useEffect marcada como async | Declare funcao async dentro do useEffect e chame-a separadamente |
