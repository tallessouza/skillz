...
metadata:
  ...
  tags: [react, virtual-dom, components, declarative-ui, jsx]
---
...
|----------|---------------|---------|
| Componente nao atualiza ao mudar dados | Estado nao configurado corretamente | Use `useState` para dados que devem refletir na tela |
| JSX retorna erro de sintaxe | Multiplos elementos raiz sem wrapper | Envolva em `<>...</>` (Fragment) ou uma div container |
| Re-renders excessivos | Estado sendo atualizado desnecessariamente | Verifique se o estado esta no componente correto (lifting state) |
| Componente nao renderiza | Componente nao exportado ou nao importado | Verifique export/import e que o nome comeca com maiuscula |
