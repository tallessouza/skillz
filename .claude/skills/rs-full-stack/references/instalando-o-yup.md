...
metadata:
  ...
  version: 1.0.0
  course: full-stack
  module: yup-validation
  tags: [yup, validation, react-hook-form, schema, forms]
---
...
|----------|-------|---------|
| TypeScript reclama de tipos com schema vazio | Schema sem campos definidos | Definir pelo menos um campo no schema ou aguardar implementacao |
| Resolver nao reconhecido | Import com path incorreto | Importar de `@hookform/resolvers/yup` (nao da raiz) |
| Validacao nao executa no submit | Resolver nao passado ao useForm | Adicionar `resolver: yupResolver(schema)` no useForm |
| Schema recriado a cada render | Declarado dentro do componente | Mover declaracao do schema para fora da funcao do componente |
