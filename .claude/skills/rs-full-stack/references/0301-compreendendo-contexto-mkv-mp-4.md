...
metadata:
  ...
  course: full-stack
  module: fundamentos
  tags: [react-context, prop-drilling, global-state, provider, useContext]
---
...
### Problem: `useContext` returns the default value instead of the provider's value
- **Cause**: The component consuming the context is rendered outside the Provider's tree
- **Fix**: Ensure the Provider wraps all components that need to access the context, typically at the `App.tsx` level around `<Routes />`
