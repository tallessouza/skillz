...
metadata:
  ...
  course: full-stack
  module: fundamentos
  tags: [react, react-hook-form, controller, forms, useForm]
---
...
|----------|---------------|---------|
| Input nao atualiza ao digitar | Esqueceu `{...field}` no render do Controller | Adicione o spread `{...field}` no elemento input |
| `data` vazio no submit | Controller sem `name` definido | Adicione prop `name` unica em cada Controller |
| Erro: `control` is undefined | Nao desestruturou `control` do `useForm()` | Use `const { control, handleSubmit } = useForm()` |
| Form submete sem coletar dados | Usando `onSubmit` direto em vez de `handleSubmit` | Use `onSubmit={handleSubmit(suaFuncao)}` no form |
