...
metadata:
  ...
  tags: [react, tailwind, input, form, component, accessibility]
---
...
|----------|-------|---------|
| Focus no input nao muda cor da legend | Falta `focus-within` no fieldset ou `text-inherit` na legend | Adicione `focus-within:text-green-100` no fieldset e `text-inherit` na legend |
| Outline padrao do browser aparece ao focar | Falta `outline-none` no input | Adicione `outline-none` e substitua por `focus:border-2 focus:border-{cor}` |
| Props nativas do input nao funcionam | Props nao repassadas com spread operator | Verifique que `{...rest}` esta no elemento `<input>` |
