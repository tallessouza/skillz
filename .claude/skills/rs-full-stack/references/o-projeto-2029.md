...
metadata:
  ...
  course: full-stack
  module: deployment
  tags: [deploy, validation, pre-deploy, frontend, local-testing]
---
...
|----------|-------|---------|
| `npm install` falha | Versao do Node incompativel | Verifique campo `engines` no `package.json` |
| `npm run dev` falha com erro de porta | Porta ja em uso por outro processo | Encerre o processo ou use porta alternativa |
| Pagina carrega mas com erros no console | Assets faltando ou imports quebrados | Corrija todos os erros antes de fazer deploy |
