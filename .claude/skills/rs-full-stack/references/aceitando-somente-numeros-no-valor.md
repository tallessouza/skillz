...
metadata:
  ...
  course: full-stack
  module: fundamentos
  tags: [javascript, regex, input, validation, forms, dom]
---
...
|----------|---------------|---------|
| Letras ainda aparecem no input | Flag `g` ausente no regex | Use `/\D/g` com o `g` global, nao apenas `/\D/` |
| Filtro nao funciona com paste | Usando evento `keydown` em vez de `input` | Troque para o evento `input` que captura paste e autocomplete |
| Ponto decimal sendo removido | `/\D/g` remove tudo que nao e digito, incluindo ponto | Use `/[^0-9.]/g` para preservar o ponto decimal |
| Input fica vazio ao digitar | Replace retornando string vazia para todo o valor | Verifique se o regex esta correto e nao ha erro de logica |
