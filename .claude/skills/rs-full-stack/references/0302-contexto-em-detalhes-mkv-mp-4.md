...
metadata:
  ...
  course: full-stack
  module: fundamentos
  tags: [react-context, provider, custom-hook, useContext, state-sharing]
---
...
### Problem: Custom hook returns empty object `{}` instead of context data
- **Cause**: The `createContext({})` default value is returned because the component is outside the Provider tree
- **Fix**: Wrap the component tree with the Provider component and pass the actual data via `value` prop
