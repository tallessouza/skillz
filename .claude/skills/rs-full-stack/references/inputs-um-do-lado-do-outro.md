...
metadata:
  ...
  course: full-stack
  module: css-layout
  tags: [tailwind, flex, gap, forms, layout, inline-fields]
---
...
|----------|---------------|---------|
| Campos empilhados em vez de lado a lado | Falta `flex` no wrapper pai | Adicionar `class="flex gap-4"` na div container |
| Espacamento irregular entre campos | Usando margin manual em vez de gap | Substituir margins por `gap-4` no container flex |
| Campos nao ocupam largura igual | Filhos sem `flex-1` | Adicionar `flex-1` nos filhos para distribuir igualmente |
| Layout quebra em telas pequenas | Flex sem responsividade | Considerar `flex-col` em breakpoints menores |

## Deep reference library

- [deep-explanation.md](references/deep-explanation.md) — Raciocinio completo sobre layout flex em formularios
- [code-examples.md](references/code-examples.md) — Exemplos expandidos com variacoes de formulario