...
metadata:
  ...
  course: full-stack
  module: dependencias
  tags: [npm, package-management, versioning, rollback]
---
...
|----------|---------------|---------|
| `npm i pacote@latest` instala versao inesperada | Tag `latest` aponta para versao que voce nao esperava | Use `npm i pacote@x.y.z` para fixar versao especifica |
| App quebra apos atualizar pacote | Breaking change na nova versao | Reverta com `npm i pacote@versao-anterior` |
| `npm outdated` ainda mostra versao antiga | Lockfile desincronizado | Execute `npm install` para sincronizar |
| Erro de compatibilidade entre pacotes | Versoes incompativeis entre dependencias | Verifique peer dependencies e atualize pacotes relacionados |
