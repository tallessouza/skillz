...
metadata:
  ...
  tags: [react, pagination, state, buttons, ui]
---
...
|----------|-------|---------|
| Pagina pula dois numeros ao clicar rapido | Usando `setPage(page + 1)` com valor stale | Use `setPage(prev => prev + 1)` com callback |
| Botao continua ativo na ultima pagina | Falta `disabled` com verificacao de limite | Adicione `disabled={current === total}` |
| Pagina vai para zero ou negativo | Sem verificacao de limite inferior | Adicione `prevPage > 1` na condicao de "previews" |
| Total de paginas nao atualiza com API | `totalOfPages` nao esta sendo atualizado com resposta | Chame `setTotalOfPages` ao receber o total da API |
