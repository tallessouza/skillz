...
metadata:
  ...
  course: full-stack
  module: fundamentos
  tags: [react, upload, file-input, tailwind, components]
---
...
|----------|---------------|---------|
| Clique no label nao abre seletor de arquivo | `htmlFor` no label nao corresponde ao `id` do input | Verifique que ambos usam o mesmo valor (ex: `upload`) |
| Nome do arquivo nao aparece apos selecao | `onChange` nao esta extraindo `e.target.files[0].name` | Adicione handler que atualiza o estado `fileName` |
| Icone de upload nao renderiza | Importacao do SVG incorreta ou caminho errado | Confirme o path do `uploadSvg` e que o bundler suporta SVG imports |
| Hover nao funciona no label | Classes de transicao ausentes | Adicione `transition ease-linear` junto com `hover:bg-{color}` |
