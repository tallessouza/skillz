...
<!-- Tailwind CSS -->
<button class="bg-blue-500 text-white px-4 py-2 rounded hover:bg-blue-600">Salvar</button>
```
...
|----------|---------------|---------|
| Classes Tailwind nao aplicam estilo | Tailwind nao instalado ou configurado no projeto | Siga o guia de instalacao do tailwindcss.com para seu framework |
| Classe customizada nao funciona | Arquivo nao incluido no `content` do tailwind.config | Adicione o path dos arquivos ao array `content` |
| Estilo aparece no dev mas nao no build | Classe gerada dinamicamente nao detectada pelo purge | Use classes completas, nunca concatene strings para formar classes |
| Conflito entre Tailwind e CSS existente | Preflight do Tailwind reseta estilos globais | Use `preflight: false` no config ou ajuste os estilos conflitantes |
