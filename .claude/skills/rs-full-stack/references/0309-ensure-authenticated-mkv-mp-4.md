...
metadata:
  ...
  course: full-stack
  module: fundamentos
  tags: [jwt, middleware, authentication, express, bearer-token, route-protection]
---
...
|----------|---------------|---------|
| `JWT token not found` em rota publica | Rota registrada apos o middleware de auth | Mover rotas publicas para ANTES de `routes.use(ensureAuthenticated)` |
| `Invalid JWT token` com token valido | Secret diferente entre sign e verify | Verificar que ambos usam `authConfig.jwt.secret` |
| `Property 'user' does not exist on type 'Request'` | Falta declaration merging | Criar `src/types/express.d.ts` com interface Request estendida |
| Token expirado retorna erro generico | Catch trata todos os erros igualmente | Comportamento esperado — "Invalid JWT token" cobre expiracao |
| `request.user` e undefined no controller | Middleware nao injeta user no request | Verificar `request.user = { id: user_id, role }` no middleware |
