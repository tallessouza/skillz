...
metadata:
  ...
  course: full-stack
  module: fundamentos
  tags: [react, pagination, tailwind, components, typescript]
---
...
|----------|---------------|---------|
| Botoes de navegacao nao funcionam | Faltam handlers onClick nos botoes prev/next | Adicione `onClick` com logica de incrementar/decrementar pagina |
| Paginacao nao atualiza a lista | Estado de pagina atual nao esta sendo passado como prop | Verifique se o componente pai gerencia e repassa `current` via state |
| Icones SVG nao aparecem | Caminho de importacao incorreto ou arquivo ausente | Confirme que os arquivos `left.svg` e `right.svg` existem em `assets/` |
| Layout desalinhado | Classes flex nao aplicadas corretamente | Use `flex flex-1 justify-center items-center gap-4` no container |
