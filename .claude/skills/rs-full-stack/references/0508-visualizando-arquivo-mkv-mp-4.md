...
metadata:
  ...
  course: full-stack
  module: fundamentos
  tags: [frontend, file-viewing, url-construction, template-literals, uploads]
---
...
|----------|---------------|---------|
| Arquivo nao abre no navegador | URL construida incorretamente | Verifique a URL completa no console e acesse manualmente no browser |
| URL quebra ao mudar de ambiente | Base URL hardcoded | Use variavel de ambiente para a base URL |
| `window.open` bloqueado pelo navegador | Pop-up blocker ativo | Use `<a href={url} target="_blank">` como alternativa |
| Arquivo retorna 404 | Nome do arquivo nao inclui extensao | Verifique se `fileURL` contem nome + extensao completa |