...
metadata:
  ...
  course: full-stack
  module: backend
  tags: [prisma, zod, validation, api, audit-log, express, node-js]
---
...
| Problema | Solução |
|----------|---------|
| **Log created for non-existent entity** | Add `findUnique` check before `create` — if the parent entity is null, throw `AppError` with 404. |
| **Log created in wrong state** | Add a status guard: verify the parent entity's current status allows the operation before proceeding. |
| **Zod validation not catching invalid UUIDs** | Use `z.string().uuid()` instead of just `z.string()` for ID fields to enforce UUID format. |
