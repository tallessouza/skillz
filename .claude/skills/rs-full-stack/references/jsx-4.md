...
metadata:
  ...
  course: full-stack
  module: react-jsx
  tags: [react, jsx, components, export, PascalCase, imports]
---
...
|----------|---------------|---------|
| Componente não renderiza (tela em branco) | Nome do componente em minúsculo — React trata como elemento HTML | Renomeie para PascalCase: `app` para `App` |
| Import falha com "module not found" | Extensão `.tsx` incluída no import | Remova a extensão: `import { App } from './App'` (sem `.tsx`) |
| `export default` permite renomeação acidental | Usando `import Qualquer from './App'` | Troque para named export: `export function App()` e `import { App }` |
