...
metadata:
  ...
  course: full-stack
  module: fundamentos
  tags: [express, multer, upload, middleware, authorization, form-data]
---
...
|----------|---------------|---------|
| `req.file` é `undefined` | Campo do form-data não corresponde ao nome em `single()` | Verifique se o campo no cliente usa exatamente o mesmo nome |
| Upload retorna 401 | Middleware de autorização bloqueia antes do Multer | Confirme que o token está sendo enviado no header |
| Arquivo não aparece na pasta `tmp/` | Configuração do Multer com path errado | Verifique `uploadConfig.multer` e o `dest`/`diskStorage` |
| `uploadConfig.multer` é undefined | Export padrão não correspondente ao import | Use `export default` no config e `import uploadConfig from` |
