...
metadata:
  ...
  course: full-stack
  module: fundamentos
  tags: [react, pagination, useEffect, filtering, state-management]
---
...
|----------|---------------|---------|
| Mudar pagina nao atualiza os dados | `page` nao esta nas dependencias do useEffect | Adicione `page` ao array de dependencias: `[page]` |
| Botao avancar ativo na ultima pagina | `totalPages` nao atualizado | Sincronize `totalPages` a partir da resposta da API |
| Filtro aplicado mas mostra pagina vazia | Pagina nao resetada para 1 ao filtrar | Adicione `setPage(1)` junto com `setSearchTerm(term)` |
| Paginacao calcula tudo no frontend | Todos os dados carregados de uma vez | Envie `page` e `perPage` como query params para a API |