...
metadata:
  ...
  course: full-stack
  module: fundamentos
  tags: [prisma, findMany, filter, relations, zod, query-params]
---
...
|----------|---------------|---------|
| `contains` não filtra corretamente | Valor do filtro tem espaços extras | Aplique `.trim()` no valor antes de usar no `contains` |
| Relação retorna `null` no include | Registro não tem relação vinculada no banco | Verifique se o `userId` existe na tabela relacionada |
| Erro de validação Zod nos query params | Parâmetro enviado com tipo errado | Use `.optional().default("")` para params opcionais |
| Ordenação não funciona | Campo do `orderBy` não existe no schema | Verifique o nome exato do campo no schema Prisma |
