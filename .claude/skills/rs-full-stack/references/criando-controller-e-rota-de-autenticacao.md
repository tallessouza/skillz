...
metadata:
  ...
  course: full-stack
  module: express-api
  tags: [express, controller, routes, authentication, sessions]
---
...
|----------|-------|---------|
| Rota retorna 404 sem erro visivel | Espaco invisivel no path ou variavel de ambiente | Inspecione a string do path com `JSON.stringify()` para revelar caracteres ocultos |
| TypeScript importa `Request` errado | Auto-import puxou tipo de outro pacote | Explicite `import { Request, Response } from 'express'` |
| Endpoint de criacao retorna 200 em vez de 201 | Esqueceu `.status(201)` antes de `.json()` | Adicione `response.status(201).json(...)` |

## Deep reference library

- [deep-explanation.md](references/deep-explanation.md) — Raciocinio completo sobre separacao controller/rota e debugging de URLs
