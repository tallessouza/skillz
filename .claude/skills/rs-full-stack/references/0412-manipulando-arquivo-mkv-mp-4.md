...
metadata:
  ...
  course: full-stack
  module: fundamentos
  tags: [node, fs, upload, disk-storage, rename, unlink]
---
...
|----------|---------------|---------|
| `ENOENT` ao mover arquivo | Arquivo não existe em `tmp/` ou path errado | Use `fs.promises.access()` para verificar existência antes |
| Pasta `uploads/` não criada | `mkdir` não chamado antes do `rename` | Adicione `fs.promises.mkdir(dest, { recursive: true })` |
| `fs.renameSync` bloqueia o servidor | Usando versão síncrona do fs | Troque para `await fs.promises.rename()` |
| Arquivo inválido não é deletado no erro | Catch não chama `deleteFile` para o arquivo temporário | Adicione limpeza no bloco catch com tipo `"tmp"` |
