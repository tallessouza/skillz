...
metadata:
  ...
  tags: [react, tailwind, header, flexbox, layout, SVG, hover]
---
...
|----------|-------|---------|
| Logo e acoes nao ficam nas extremidades | Falta `justify-between` no container flex | Adicione `className="w-full flex justify-between"` no `<header>` |
| SVG nao aparece na tela | Import incorreto ou path errado | Verifique o caminho relativo: `import logo from "../assets/logo.svg"` |
| Hover nao tem transicao suave | Falta `transition` junto com `hover:opacity` | Use `hover:opacity-75 transition ease-linear` juntos |
| Icones e texto desalinhados verticalmente | Falta `items-center` no container dos icones | Adicione `flex items-center gap-3` na div que agrupa texto e icones |
