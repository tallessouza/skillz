...
- name: Deploy to AppRunner
  ...
  with:
    ...
    image: ${{ secrets.AWS_ACCOUNT_ID }}.dkr.ecr.us-east-1.amazonaws.com/rocketseat-api:${{ github.sha }}
...
- name: Build Docker Image
  run: docker build -t $ECR_REGISTRY/rocketseat-api:${{ github.sha }} .

- name: Push Docker Image
  run: docker push $ECR_REGISTRY/rocketseat-api:${{ github.sha }}

- name: Tag as latest
  run: docker tag $ECR_REGISTRY/rocketseat-api:${{ github.sha }} $ECR_REGISTRY/rocketseat-api:latest

- name: Push latest tag
  run: docker push $ECR_REGISTRY/rocketseat-api:latest
```
