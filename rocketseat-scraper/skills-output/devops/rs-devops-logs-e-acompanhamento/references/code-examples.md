  ...
  build-and-deploy:
    ...
    steps:
      ...
      - name: Build Docker Image
        run: |
          docker build -t ${{ steps.login-ecr.outputs.registry }}/rocketseat-api:${{ github.sha }} .

      - name: Push Docker Image
        run: |
          docker push ${{ steps.login-ecr.outputs.registry }}/rocketseat-api:${{ github.sha }}

      - name: Tag as latest
        run: |
          docker tag \
            ${{ steps.login-ecr.outputs.registry }}/rocketseat-api:${{ github.sha }} \
            ${{ steps.login-ecr.outputs.registry }}/rocketseat-api:latest

      - name: Push latest tag
        run: |
          docker push ${{ steps.login-ecr.outputs.registry }}/rocketseat-api:latest
      ...
      - name: Deploy to AppRunner
        ...
        with:
          ...
          image: ${{ steps.login-ecr.outputs.registry }}/rocketseat-api:${{ github.sha }}
...
Ao alterar a resposta do servico (ex: "Hello World" para "Ola, Rocket City"), o teste automatizado tambem deve ser atualizado:
...
describe('AppController', () => {
  it('should return greeting', () => {
    expect(appController.getHello()).toBe('Olá, Rocket City');
  });
});
```
