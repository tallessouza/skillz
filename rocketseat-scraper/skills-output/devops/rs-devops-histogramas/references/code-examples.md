...
```promql
sum(rate(http_client_duration_milliseconds_bucket{service_name="api-rocketseat"}[5m]))
/
sum(rate(http_client_duration_milliseconds_count{service_name="api-rocketseat"}[5m]))
```

### Media nos ultimos 10 minutos

```promql
sum(rate(http_client_duration_milliseconds_bucket{service_name="api-rocketseat"}[10m]))
/
sum(rate(http_client_duration_milliseconds_count{service_name="api-rocketseat"}[10m]))
```
...
| Label | Exemplo | Uso |
|-------|---------|-----|
| `service_name` | `api-rocketseat` | Isolar por aplicacao |
