...
```bash
docker build -t ns-api-rocket:v9 .
```

Verificando tamanho da imagem:
```bash
docker images ns-api-rocket
...
```bash
# App na rede custom
docker run -d --network primeira-network -p 3001:3000 ns-api-rocket:v9
```
...
Rebuild:
```bash
docker build -t ns-api-rocket:v10 .
```

### 7. Subindo app corrigida

```bash
docker run -d --network primeira-network -p 3001:3000 ns-api-rocket:v10
```
