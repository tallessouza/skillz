...
Quando voce duplica informacao (copiar o nome do resource group como string), qualquer mudanca no nome exige alterar em multiplos lugares. O instrutor demonstrou isso ao vivo: ao corrigir um typo no nome do Resource Group ("RocketCity" → "Rocketseat"), todos os recursos que referenciam `azurerm_resource_group.rg.name` atualizam automaticamente.
