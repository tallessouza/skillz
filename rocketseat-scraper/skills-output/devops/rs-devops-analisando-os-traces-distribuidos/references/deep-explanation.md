...
O conceito central dos traces distribuidos e a propagacao do Trace ID. Quando uma requisicao nasce no App 1 (ex: app-rocketseat), ela recebe um Trace ID unico. Ao chamar o App 2 (ex: app-rocketseat-2), esse Trace ID e propagado via headers HTTP. Cada servico cria seus proprios spans dentro desse trace, permitindo visualizar a cadeia completa no Grafana Tempo.

O instrutor destaca: "a requisicao ela nasceu no app Rocketseat com um Trace ID especifico e ela morreu no app Rocketseat 2 com outro Trace ID" — mas o ponto crucial e que o **parent trace** conecta ambos, permitindo a visualizacao end-to-end.
