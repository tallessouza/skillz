...
Estrutura chave-valor simples. A chave (`APP`) e o que voce usa no `process.env`, o valor (`RocketseatApp`) e o que sera retornado.
...
export class AppService {
  getHello(): string {
    console.log(process.env.APP); // "RocketseatApp"
    return 'Hello World!';
  }
}
```
