...
- **`extends`**: Herda configuracoes completas (regras + plugins + settings). O `@rocketseat/eslint-config/node` ja vem com regras de TypeScript, import order, etc. O `plugin:drizzle/all` ativa todas as regras do plugin Drizzle.
