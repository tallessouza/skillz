...
### Step 4: Configurar ESLint da Rocketseat

```bash
pnpm add -D @rocketseat/eslint-config
```

```json
// .eslintrc.json
{
  "extends": ["next/core-web-rules", "@rocketseat/eslint-config/next"]
}
```
...
| Usar ESLint padrao do Next sem extensao | Instalar `@rocketseat/eslint-config` para regras mais completas |
