...
O `validationSchema` do ConfigModule usa Joi por padrao. Como o ecossistema Rocketseat usa Zod, a opcao `validate` e mais adequada — ela recebe uma funcao que voce controla totalmente. O `envSchema.parse()` do Zod faz throw se invalido, que e exatamente o que o ConfigModule espera.
