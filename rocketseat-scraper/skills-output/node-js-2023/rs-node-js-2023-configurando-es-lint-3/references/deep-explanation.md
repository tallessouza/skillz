...
Isso e util para entender o que existe, mas na pratica voce configura uma vez e reutiliza. A config `@rocketseat/eslint-config` encapsula todas essas decisoes com os padroes que o Mayk (instrutor) prefere, equivalente ao que ele selecionaria manualmente no wizard.
...
Essas preferencias estao encapsuladas no pacote `@rocketseat/eslint-config`.
