...
description: "Guides deployment platform selection and Render setup when hosting Node.js backends and databases. Use when user asks to 'deploy my app', 'host my backend', 'set up Render', 'choose a hosting platform', or 'put my API in production'. Covers free tier limitations, 90-day database expiration, and account creation flow. Make sure to use this skill whenever evaluating deployment platforms or setting up Render for the first time. Not for CI/CD pipelines, Docker configuration, Kubernetes orchestration, or frontend-only static hosting."
---
