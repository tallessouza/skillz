...
- Texto literal como `rocketseat` — exatamente essa sequencia

Exemplo: `.+@rocketseat\.com` significa:
1. `.+` — um ou mais caracteres quaisquer (a parte antes do @)
2. `@` — literal
3. `rocketseat` — exatamente "rocketseat"
4. `\.` — ponto literal
5. `com` — exatamente "com"

Isso restringe o campo para aceitar apenas emails do dominio `@rocketseat.com`.

## A importancia do title com pattern

Quando o pattern falha na validacao, o navegador mostra uma mensagem generica como "Please match the requested format". O atributo `title` substitui essa mensagem por algo util para o usuario, como "Apenas emails @rocketseat.com". Sem o `title`, o usuario nao sabe o que esta errado.
