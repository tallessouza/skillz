...
develop-story:
  implementation-workflow: |
    BEFORE writing any implementation code, follow the rs-implementation-workflow skill phases:

    Phase 1 — Domain Modeling (DDD):
      - Identify domain, subdomains, entities, value objects, use cases
      - Name entities by subdomain context (never generic "User")
      - Load rs-clean-code DDD references for patterns
      - Output: domain model summary (entities, VOs, use cases)

    Phase 2 — Architecture (SOLID + Clean Arch):
      - Apply SRP (connector "and" test), OCP (extension over modification), DIP (inject deps)
      - Define layers: Domain → Application → Infrastructure
      - Define interfaces/contracts between layers
      - Load rs-clean-code SOLID references + rs-node-js Clean Architecture
      - Output: folder structure, main interfaces, dependency graph

    Phase 3 — Implementation (Clean Code):
      - NOW write code following naming, conditionals, component patterns
      - Load specific skill routers based on tech stack (rs-node-js, rs-next-js, rs-api-com, etc.)
      - Apply all clean code rules from rs-clean-code

    Phase 4 — Validation:
      - Self-review the code
      - Run CodeRabbit pre-commit
      - Run tests

    SKIP RULES:
      - Bug fix (1-3 lines): skip phases 1-2, go to phase 3
      - Add field to existing entity: skip phase 1, start at 2
      - New feature/module: ALL phases mandatory
      - Refactoring: skip phase 1, start at 2
  order-of-execution: 'Read (first or next) task→Follow implementation-workflow phases→Implement Task and its subtasks→Write tests→Execute validations→Only if ALL pass, then update the task checkbox with [x]→Update story section File List to ensure it lists and new or modified or deleted source file→repeat order-of-execution until complete'
...
dependencies:
  ...
  skillz_skills:
    enabled: true
    description: |
      ~1,067 skills extracted from Skillz courses, organized as 11 router skills
      with progressive disclosure (router → individual skill → deep references).
    architecture: |
      .claude/skills/rs-{course}/SKILL.md     — Router (auto-discovered, ~400 char description)
      .claude/skills/rs-{course}/references/  — Individual skills (loaded on demand)
      data/skills/{course}/rs-*/              — Raw skills with deep-explanation + code-examples
    routers:
      - rs-clean-code       # 21 skills: SOLID, DDD, naming, components
      - rs-node-js          # 272 skills: Streams, HTTP, Fastify, NestJS, DDD
      - rs-next-js          # 174 skills: App Router, SSR, API Routes
      - rs-devops           # 223 skills: Docker, K8s, Terraform, CI/CD
      - rs-saa-s            # 106 skills: Monorepo, RBAC, auth, permissions
      - rs-testes-e         # 74 skills: Unit, E2E, Playwright, CI
      - rs-seguranca-para   # 54 skills: Auth, XSS, CSRF, injection
      - rs-ia-node          # 50 skills: LLM, embeddings, function calling
      - rs-masterizando     # 37 skills: Tailwind CSS patterns
      - rs-api-com          # 34 skills: API with Bun, Drizzle ORM
      - rs-redux-zustand    # 22 skills: State management patterns
    workflow: |
      During implementation:
      1. Skills activate automatically via description matching (auto-discovery)
      2. When a router activates, read its SKILL.md to find the specific sub-skill
      3. Navigate to references/{skill}.md for complete rules and examples
      4. If deeper understanding needed, follow links to data/skills/ deep-explanation.md
      5. Apply the skill's rules to the code being written
      6. After implementation, CodeRabbit validates the result
    integration_with_coderabbit: |
      Skills inform HOW to write code (proactive, during generation).
      CodeRabbit validates AFTER code is written (reactive, pre-commit).
      Together: Skills → Write code → CodeRabbit review → Fix if needed → Commit
