...
### Step 4: Plan Approach (with Skill Consultation)

```yaml
step_4:
  id: '4'
  name: 'Plan Approach'
  description: 'Create detailed implementation approach by consulting skills FIRST'

  actions:
    # 4a: SKILL ROUTING — Antes de qualquer planejamento
    - action: consult_skill_router
      description: |
        OBRIGATÓRIO: Abrir rs-implementation-workflow/SKILL.md e determinar
        quais fases se aplicam a esta subtask.
      steps:
        - Abrir .claude/skills/rs-implementation-workflow/SKILL.md
        - Identificar se subtask requer Fase 1 (DDD), Fase 2 (SOLID), Fase 3 (Implementação)
        - Para cada fase aplicável, navegar até o skill router indicado
        - Carregar as references/ específicas que serão usadas
        - REGISTRAR quais skills foram consultadas no approach output

    # 4b: Para cada arquivo que será criado/modificado, identificar skills aplicáveis
    - action: map_skills_to_files
      description: |
        Para cada arquivo no plano, identificar QUAL skill reference guia a implementação:
        - Criando entidade? → rs-node-js references/2023-entidades-e-casos-de-uso.md
        - Criando use case? → rs-node-js references/2023-caso-de-uso-criar-*.md
        - Criando controller? → rs-node-js references/2023-controller-de-*.md
        - Criando componente React? → rs-clean-code references/componentes-puros.md
        - Nomeando variáveis? → rs-clean-code references/nomenclatura-de-variaveis-*.md
        - Escrevendo condicionais? → rs-clean-code references/regras-em-condicionais.md
        - Configurando projeto? → rs-node-js references/2023-criando-um-projeto-node.md
        O mapeamento skill→arquivo DEVE existir no output do approach.
    ...
    - action: plan_changes
      for_each_file:
        ...
        - skill_ref: 'WHICH skill reference guides this change'

    - action: identify_tests
      unit: 'Unit tests needed'
      integration: 'Integration tests if applicable'
      skill_ref: 'rs-node-js → ramo "Escrevendo testes" ou rs-testes-e'

  validation:
    check: 'Approach documented with specific file changes AND skill references mapped'
    onFailure: retry
  ...
  output:
    approach:
      ...
      changes: array
      tests: array
      skillsConsulted: array   # NOVO: lista de skills consultadas
      skillFileMap: object     # NOVO: { "src/entity.ts": "rs-node-js/references/2023-entidades-e-casos-de-uso.md" }
```

### Step 5: Write Code (Skill-Guided)

```yaml
step_5:
  id: '5'
  name: 'Write Code'
  description: 'Implement following skills progressively — consult skill reference BEFORE each micro-action'

  actions:
    - action: implement
      follow: approach
      style: 'Match skill references + existing codebase patterns'

    - action: for_each_file
      in: approach.files
      do:
        # NOVO: Carregar a skill reference mapeada no Step 4 ANTES de escrever
        - load_skill_reference: approach.skillFileMap[file]
        - create_or_modify: file
        - apply_changes: approach.changes[file]
        - preserve: 'Existing functionality'

    # NOVO: Consulta progressiva durante a escrita
    - action: progressive_skill_check
      description: |
        DURANTE a escrita de código, para CADA micro-decisão:

        Nomeando uma variável?
          → Carregar rs-clean-code/references/nomenclatura-de-variaveis-*.md
          → Seguir: nomes descritivos, sem abreviações, contexto claro

        Nomeando um booleano?
          → Carregar rs-clean-code/references/causa-vs-efeito.md
          → Seguir: nome descreve a causa (isExpired), não o efeito (shouldRedirect)

        Escrevendo um condicional (if/else)?
          → Carregar rs-clean-code/references/regras-em-condicionais.md
          → Seguir: early return, sem else, sem nesting

        Usando número literal (magic number)?
          → Carregar rs-clean-code/references/numeros-magicos.md
          → Extrair para constante nomeada

        Fazendo conversão de tipo (!!, +, '')?
          → Carregar rs-clean-code/references/evite-syntatic-sugars.md
          → Usar método explícito (Number(), String(), Boolean())

        Escrevendo parâmetros de função (>2)?
          → Carregar rs-clean-code/references/parametros-e-desestruturacao.md
          → Usar desestruturação de objeto

        Criando componente React?
          → Carregar rs-clean-code/references/componentes-puros.md
          → Sem side effects no render, composição > customização

        Escrevendo JSX condicional?
          → Carregar rs-clean-code/references/condicionais-no-render.md
          → Sem ternários aninhados, extrair para variável ou componente

        Criando entidade/VO?
          → Carregar rs-node-js/references/2023-entidades-e-casos-de-uso.md
          → Classe com validação no create(), ID como ValueObject

        Criando caso de uso?
          → Carregar rs-node-js/references/2023-caso-de-uso-criar-*.md (pattern matching)
          → Input DTO, Either pattern para erros, injeção de repositório

        Criando repositório?
          → Carregar rs-node-js/references/2023-repository-pattern.md
          → Interface no domain, implementação na infra

        Escrevendo teste?
          → Carregar rs-node-js/references/2023-primeiro-teste-unitario.md
          → describe/it, in-memory repository, factory pattern

        A skill reference é o MANUAL. O código é o OUTPUT.
        Se não existe skill para a ação → seguir padrões do codebase.

  validation:
    check: 'Code compiles/loads without syntax errors AND follows skill patterns'
  ...
  output:
    ...
    skillsUsed: array  # NOVO: quais references foram efetivamente consultadas
```
...
step_5_5:
  ...
  checklist:
    ...
    - id: "SC-5.5.6"
      question: "O código segue os padrões das skills consultadas?"
      action: |
        Para cada skill reference carregada no Step 4/5:
        - Variáveis nomeadas conforme nomenclatura-de-variaveis?
        - Condicionais seguem regras-em-condicionais (early return, sem else)?
        - Booleanos nomeados por causa (causa-vs-efeito)?
        - Sem magic numbers (numeros-magicos)?
        - Sem syntactic sugars em conversões (evite-syntatic-sugars)?
        - Entidades com create() e validação (entidades-e-casos-de-uso)?
        - Use cases com Either pattern (functional-error-handling)?
        - Componentes puros sem side effects (componentes-puros)?
        Se QUALQUER violação: corrigir antes de prosseguir.
