...
1. **Initialization** (On Yolo Mode Start)
   ...
   - **SKILL ROUTING**: Abrir rs-implementation-workflow/SKILL.md, identificar fases aplicáveis, mapear skill references por arquivo/ação
...
2. **Task Execution** (Autonomous loop)
   - Read next task
   - **Consult skill references mapeadas no Step 1 para esta task**
   - **A cada micro-ação (nomear, condicional, criar classe), navegar a decision tree da skill relevante**
...
1. **Story Analysis** (With User)
   - Read story file completely
   - **SKILL ROUTING**: Abrir rs-implementation-workflow/SKILL.md, identificar fases, mapear skills por task/arquivo
   - Present summary of tasks, AC, **e quais skills serão consultadas**
   - Confirm understanding with user

2. **Task Execution** (Interactive loop)
   - Read next task
   - **Carregar skill references mapeadas para esta task**
   - **Decision Checkpoints** (Prompt user at — **informados pelas skills**):
...
2. **Skill Discovery Phase (OBRIGATÓRIO)**
   - Abrir `.claude/skills/rs-implementation-workflow/SKILL.md`
   - Para cada task da story, navegar a decision tree e identificar:
     - Quais fases do workflow se aplicam (DDD, SOLID, Implementação, Validação)
     - Quais skill routers serão consultados (rs-node-js, rs-clean-code, etc.)
     - Quais references/ específicas guiam cada ação
   - **O plano DEVE incluir um mapeamento skill→ação para cada arquivo/componente**
   - Exemplo: `src/config.js → rs-node-js/references/2023-tratando-env-com-zod.md`
   - Exemplo: `src/dispatcher.js → rs-node-js/references/2023-entendendo-streams-no-node.md`

3. **Questionnaire Generation**
   - Generate comprehensive questions covering:
     - Architecture decisions (informadas pelas skills de DDD/SOLID)
     - Library and framework choices (informadas pelas skills da stack)
     - Algorithm and data structure selections
     - Testing strategy (informada por rs-testes-e / rs-node-js testes)
     - Edge case handling
     - Performance requirements
     - Security considerations (informada por rs-seguranca-para)
...
4. **Execution Plan Creation**
   ...
   - **Incluir seção "Skills Map" no plano**: lista de skill references que serão usadas por arquivo
   - Present plan to user for approval
   - Wait for user confirmation before proceeding

5. **Zero-Ambiguity Execution (Skill-Guided)**
   - Execute tasks with full context from questionnaire
   - No additional decision points (all decided in pre-flight)
   - Implement task and subtasks **seguindo as skill references mapeadas**
   - A cada micro-ação (nomear variável, escrever função), consultar a reference indicada
   - Write tests
   - Execute validations
   - Mark task complete [x] only if ALL validations pass
   - Update File List
...
2. **SKILL ROUTING (OBRIGATÓRIO):**
   - Abrir `.claude/skills/rs-implementation-workflow/SKILL.md`
   - Determinar quais fases (DDD → SOLID → Implementação → Validação) se aplicam
   - Navegar até os skill routers indicados para a stack da task
   - Carregar as references/ específicas
   - Mapear: para cada arquivo/ação, QUAL skill reference guia a implementação
3. Implement task and its subtasks **seguindo as skills progressivamente**:
   - A cada micro-decisão (nomear variável, escrever condicional, criar classe), consultar a skill reference correspondente
   - A skill reference é o MANUAL — o código é o OUTPUT
   - Se não existe skill para a ação → seguir padrões do codebase
4. Write tests (consultar rs-node-js ramo "Escrevendo testes" ou rs-testes-e)
